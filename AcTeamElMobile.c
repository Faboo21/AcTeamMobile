...
int calculateGasCost(int ax, int ay, int vx, int vy, char target_terrain) {
    int gas_cost = (ax * ax) + (ay * ay) + (int)sqrt((3.0 * (vx * vx + vy * vy)) / 2.0);
    if (target_terrain == '~') {
        gas_cost += 1;
    }
    return gas_cost;
    ...
    for (depth = 0; depth < MAX_DEPTH; depth++) {
        ...
        for (i = 0; i < current_beam_size; i++) {
            ...
            for (m = 0; m < num_moves; m++) {
                ...
                /* Le score est maintenant une combinaison de la distance ET du carburant consommé. */
                /* 'current.gas' est l'essence au début du tour, 'candidate.gas' est l'essence dans ce futur simulé. */
                candidate.score = heatmap[candidate.y][candidate.x] + ((current.gas - candidate.gas) * 5);
